...
def train_one_epoch_memory(model, dataloader, criterion, optimizer, device, actual_seq_len):
    ...
    for batch in loop:
        ...
        # divide dimension 1 (T) in n slice of actual_seq_len.
        # For example, if T=160 and actual_seq_len=8, we will have 20 slices.
        ...
        # slice and iterate over the slices. I need to give the slices in order to the model to keep the memory.
        prev_memory = None
        full_clip_loss = 0.0
        total_loss = 0.0
        optimizer.zero_grad()
        for i in range(num_slices):
            ...
            batch_slice = {
                ...
                "ego_labels": batch["ego_labels"][:, start_idx:end_idx]  # → (B, actual_seq_len)
            }

            # put to device
            batch_slice["labels"] = batch_slice["labels"].to(device)
            batch_slice["ego_labels"] = batch_slice["ego_labels"].to(device)
            ...
            logits_flat = logits.view(B_slice * T_slice, 7)              # → (B*T, 7)
            targets_flat = batch_slice["ego_labels"].view(B_slice * T_slice)  # → (B*T,)
            # Compute loss
            loss = criterion(logits_flat, targets_flat)
            total_loss += loss
            full_clip_loss += loss.item()
        total_loss.backward()
        optimizer.step()
        running_loss += full_clip_loss
        loop.set_postfix(loss=full_clip_loss)

    print(f"Running loss for the epoch: {running_loss:.4f}")
    print(f"Length of the dataloader: {len(dataloader)}")
    avg_loss = running_loss / len(dataloader)

    return avg_loss

def train(model, dataloader_train, dataloader_val, criterion, optimizer, device, num_epochs, patience=10, actual_seq_len=8):
    best_mAP = -float('inf')
    epochs_no_improve = 0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(num_slices):
                ...
                preds = activation(logits)  
                B, T, _ = logits.shape
                all_gts.append(batch_slice["ego_labels"].view(-1).cpu().numpy())
                all_dets.append(preds.view(-1, 7).cpu().numpy())
...
def main():
    ...
    # ----------------------------
    model = FrameMemoryTransformer(num_classes=args.NUM_CLASSES, memory_size=args.SEQ_LEN)
    criterion = ego_loss
    ...
    # Training
    # ----------------------------
    train(model, dataloader_train,dataloader_val, criterion, optimizer, device, num_epochs, patience, args.ACTUAL_SEQ_LEN)
